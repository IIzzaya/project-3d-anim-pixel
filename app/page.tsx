...
export default function Home() {
  ...
  return <TooltipProvider delay={250}><main className={`studio ${immersive ? 'immersive' : ''}`}>
    <header className="topbar">{/* vinext 的 Link 会自动附加 basePath，这里保持根路径即可 */}
    ...
    <section className="workspace" aria-label="精灵预览工作区">
      ...
      <section className="presets-section" aria-label="风格预设"><div className="section-heading"><span>从一种灵感开始</span><span>04 PRESETS</span></div><div className="preset-grid">{presets.map((item, index) => <button key={item.id} className={`preset-card preset-${item.id} ${preset === item.id ? 'selected' : ''}`} onClick={() => selectPreset(item.id)} aria-pressed={preset === item.id}><div className="preset-art"><img src={`${BASE_PATH}/presets/${item.id}.png`} width={300} height={180} alt="" loading="lazy" /><span className="preset-index">0{index + 1}</span>{preset === item.id && <span className="preset-check"><Check size={11} /></span>}</div><div className="preset-description"><span>{item.name}</span><small>{item.subtitle}</small></div></button>)}</div></section>
    </section>
    ...
    <Dialog open={reference} onOpenChange={setReference}><DialogContent className="reference-dialog"><DialogTitle>参考画面</DialogTitle><DialogDescription>来自 ref.mp4：蓝色光球、半透明翅膀，以及颗粒化的光晕。</DialogDescription><img src={`${BASE_PATH}/reference.jpg`} width={760} height={630} alt="参考视频中的蓝色发光精灵，具有两对半透明翅膀" /><p className="reference-caption">当前场景以程序化 3D 几何与实时特效重建，无需外部模型。</p></DialogContent></Dialog>
